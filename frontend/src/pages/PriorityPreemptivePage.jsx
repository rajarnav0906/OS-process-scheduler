...
export default function PriorityPreemptivePage() {
  ...
  const addProcess = () => {
    setProcesses((prev) => [
      ...prev,
      { pid: `P${prev.length + 1}`, arrival: 0, burst: 1, priority: 1 },
    ]);
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      const res = await axios.post(
        `${import.meta.env.VITE_BACKEND_URL}/api/schedule/priority-preemptive`,
        payload
      );
      setResult(res.data);
      setAnimating(true);
      ...
      <div className="relative z-10 w-full space-y-6 sm:space-y-10 flex-1">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <h1 className="text-xl sm:text-3xl font-bold text-cyan-400 flex items-center gap-2 drop-shadow-md">
            <ActivitySquare className="w-6 sm:w-7 h-6 sm:h-7" />
            Priority (Preemptive) Scheduling Visualizer
          </h1>
