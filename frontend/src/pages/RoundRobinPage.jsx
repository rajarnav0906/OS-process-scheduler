...
import { Plus, Play, Trash2, Clock, Home, Loader2, AlertCircle } from "lucide-react";
    ...
    >
      ...
      <div className="relative z-10 w-full space-y-6 sm:space-y-10 flex-1">
        ...
        <motion.div
          initial={{ y: 20, opacity: 0 }}
          animate={{ y: 0, opacity: 1 }}
          className="bg-[#101827]/90 backdrop-blur border border-gray-700 rounded-xl p-4 sm:p-6 shadow-xl flex items-center gap-3"
        >
          <label className="text-cyan-300 font-medium text-sm sm:text-base">Time Quantum:</label>
          <input
            type="number"
            value={quantum}
            onChange={(e) => setQuantum(e.target.value)}
            className="w-16 sm:w-20 px-2 py-1 rounded bg-gray-900 border border-gray-600 text-center"
          />
        </motion.div>

        {/* Visualization + Loader + Fallback */}
        <div className="bg-[#101827]/90 backdrop-blur border border-gray-700 rounded-xl p-6 shadow-xl min-h-[200px] flex items-center justify-center">
          ...
          {!loading && result && (
            <motion.div initial={{ y: 20, opacity: 0 }} animate={{ y: 0, opacity: 1 }} className="w-full">
              ...
              {/* Metrics */}
              <div className="overflow-x-auto">
                <table className="w-full text-xs sm:text-sm mb-4 min-w-[420px]">
                  <thead>
                    <tr className="bg-[#0a0f1a] text-gray-300 uppercase text-[10px] sm:text-xs">
                      <th className="p-2 text-center">PID</th>
